...
<div class="masterstudy-course-player-assignments<?php echo esc_attr( 'draft' === $current_template ? ' masterstudy-course-player-assignments_draft' : '' ); ?>">
	...
	<?php if ( 'draft' === $current_template ) { ?>
		...
		STM_LMS_Templates::show_lms_template(
			...
			array(
				'assignment_id'     => $data['template_data'][ $current_template ]['id'],
				'instructor_review' => false,
				'dark_mode'         => $data['dark_mode'],
			)
		);
	} elseif ( 'draft' !== $current_template ) {
		?>
	...
	if ( ( 'passed' === $current_template || 'unpassed' === $current_template ) && ( ! empty( $data['template_data'][ $current_template ]['meta']['editor_comment'][0] ) || ! empty( $data['instructor_attachments'] ) ) ) {
		STM_LMS_Templates::show_lms_template(
